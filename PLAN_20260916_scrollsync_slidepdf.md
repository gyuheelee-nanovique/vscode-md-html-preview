...
### 교수자 보고 「4-1 덱에서 텍스트와 프리뷰가 1~2 슬라이드 어긋난다」 (같은 날 저녁)
- 실측(정적 시뮬레이션, 719줄·41장): 옛 매핑(장의 «첫 스탬프 블록 줄» 기준)은 구분선 다음 빈 줄 40개만 한 장 앞으로 틀렸다 — 1~2장 어긋남의 주범이 아니다. 주범은 **편집기 «뷰포트 중앙 줄»로 동기하는 것**: 4-1 은 한 장이 15줄 안팎이라 40~60줄 뷰포트가 3~4장을 걸치고, 커서가 있는 장과 중앙 줄의 장이 1~2장 다르다. NOTE 한 줄이 중앙에 오면 마스크로 얼어붙는 것(41/719)도 더해진다.
- 처방: 슬라이드 모드에서는 **커서 줄**로 동기(커서가 화면 안이면 커서, 아니면 중앙; 선택 이동 이벤트도 동기; 주석 마스크 없음). 덱 쪽은 `hr.slide-sep` 의 `data-source-line` 으로 「구분선 ≤ 줄」 개수 = 장 번호로 매핑(빈 장 걸러낸 뒤 `data-raw` 로 대응).
- 검증(`_slidesync_test.js`): 3줄 간격 240줄 전부 참 장과 일치(0 어긋남); 부분 갱신·프레임·메뉴 전환 회귀 통과.
